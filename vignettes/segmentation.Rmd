...
# Getting started
Image segmentation is the process of partitioning a digital image into multiple segments (sets of pixels or image objects). In the context of plant image analysis, segmentation is used to simplify the representation of an image into something that is easier to analyze. For example, when using `count_objects()` to count crop grains, first the grains need to be isolated (segmented) from the background. In `pliman` the following functions can be used to segment an image.
